...
#End Region
    ...
    Private Sub frmPatientsTwo_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
...
#End Region
    ...
    Private Sub ebnSaveUpdate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ebnSaveUpdate.Click
        ...
        Dim oBillModesID As New LookupDataID.BillModesID()
        Dim oFingerprintDeviceID As New LookupCommDataID.FingerprintDeviceID()
        Dim oPatientConsent As New SyncSoft.SQLDb.PatientConsent()
        ...
        Try
            ...
            Dim lPatientConsent As New List(Of DBConnect)
            Dim oVariousOptions As New VariousOptions()
            ...
            With oPatients
                ...
                .LastName = StringEnteredIn(Me.stbLastName, "Surname (Local Name)!")
                ...
                If oVariousOptions.ForcePatientAddress Then
                    .Phone = StringEnteredIn(Me.stbPhone, "Phone No")
                ...
                .ReferringFacility = StringMayBeEnteredIn(Me.stbReferringFacility)
                ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            ...
            ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            If oVariousOptions.ForcePatientAddress() Then
                With oPatientConsent


                    .PatientNo = RevertText(StringEnteredIn(Me.stbPatientNo, "Patient No!"))
                    .PhoneNo = StringEnteredIn(Me.stbPhone, "Phone No!")
                    .Notes = "Consented at Patient Registration"
                ...
                End With
                '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                lPatientConsent.Add(oPatientConsent)
            End If
            ...
            Select Case Me.ebnSaveUpdate.ButtonText
                ...
                Case ButtonCaption.Save
                    ...
                    If chkReceiveMedicalSMS.Checked = True And (stbPhone.Text IsNot Nothing AndAlso Not String.IsNullOrEmpty(stbPhone.Text)) Then
                        transactions.Add(New TransactionList(Of DBConnect)(lPatientConsent, Action.Save))
                    End If
...
#Region "Print FaceSheet"
    ...
    Private Sub docFaceSheet_PrintPage(ByVal sender As Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles docFaceSheet.PrintPage
        ...
        Try
            ...
            Dim referenceNo As String = StringMayBeEnteredIn(Me.stbReferenceNo)
            Dim nin As String = StringMayBeEnteredIn(Me.stbNIN)

            If Not String.IsNullOrEmpty(oName) Then
                fullNameFacesheet = lName + " " + oName + " " + fName
            Else : fullNameFacesheet = lName + " " + fName
            End If
            ...
            With e.Graphics
                ...
                If pageNo < 2 Then
                    ...
                    .DrawString("Reference No: ", bodyNormalFont, Brushes.Black, xPos + widthTopThird, yPos)
                    .DrawString(referenceNo, bodyBoldFont, Brushes.Black, widthTopFourth, yPos)
                    yPos += lineHeight
    ...
    Private Sub SetfacesheetReportPrintData()
        ...
        Try
            ...
            surgicalData.Append(ControlChars.NewLine)
            surgicalData.Append(ControlChars.NewLine)
            surgicalData.Append("Signature (Patient/Parent/Next of Kin):   " + GetCharacters("."c, 20))
            surgicalData.Append(ControlChars.NewLine)
            surgicalData.Append(ControlChars.NewLine)
            ...
            dischargeData.Append(ControlChars.NewLine)
            dischargeData.Append(ControlChars.NewLine)
            dischargeData.Append("Signature (Patient/Parent/Next of Kin):   " + GetCharacters("."c, 30))
            dischargeData.Append(ControlChars.NewLine)
            dischargeData.Append(ControlChars.NewLine)
            ...
            amputationData.Append(ControlChars.NewLine)
            amputationData.Append("Signature (Patient/Parent/Next of Kin):   " + GetCharacters("."c, 30))
            amputationData.Append(ControlChars.NewLine)
            amputationData.Append(ControlChars.NewLine)
...
#Region "Print Patient Bio Data"
    ...
    Private Sub SetBioPrintData()
        ...
        Try
            ...
            ' '''''''''''''''BIO DATA'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            'Dim bioDataTitle As New System.Text.StringBuilder(String.Empty)
            'bioDataTitle.Append(ControlChars.NewLine)
            'bioDataTitle.Append("BIO DATA: ".ToUpper())
            'bioDataTitle.Append(ControlChars.NewLine)
            'bioDataTitle.Append(ControlChars.NewLine)
            'bioDataParagraphs.Add(New PrintParagraps(bodyBoldFont, bioDataTitle.ToString()))
            'bioDataParagraphs.Add(New PrintParagraps(bodyNormalFont, Me.ClinicalFindingsData()))



            ''''''''''''''''Allergies'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            'Dim allergiesTitle As New System.Text.StringBuilder(String.Empty)
            'allergiesTitle.Append(ControlChars.NewLine)
            'allergiesTitle.Append("Allergies: ".ToUpper())
            'allergiesTitle.Append(ControlChars.NewLine)
            'allergiesTitle.Append(ControlChars.NewLine)

            'Dim tableAllergies As New System.Text.StringBuilder(String.Empty)
            'tableAllergies.Append("No: ".PadRight(padLineNo))
            'tableAllergies.Append("Allergy: ".PadRight(padService))
            'tableAllergies.Append("Reaction: ".PadRight(padNotes))
            'tableAllergies.Append(ControlChars.NewLine)
            'tableAllergies.Append(ControlChars.NewLine)

            'If Not String.IsNullOrEmpty(Me.AllergiesData()) Then
            '    bioDataParagraphs.Add(New PrintParagraps(bodyBoldFont, allergiesTitle.ToString()))
            '    If Me.dgvPatientAllergies.RowCount > 1 Then
            '        bioDataParagraphs.Add(New PrintParagraps(bodyBoldFont, tableAllergies.ToString()))
            '    End If
            '    bioDataParagraphs.Add(New PrintParagraps(bodyNormalFont, Me.AllergiesData()))
            'End If
